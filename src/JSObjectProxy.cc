...
bool keyToId(PyObject *key, JS::MutableHandleId idp) {
  if (PyUnicode_Check(key)) { // key is str type
    JS::RootedString idString(superGlobalContext.getJSContext());
    Py_ssize_t length;
    const char *keyStr = PyUnicode_AsUTF8AndSize(key, &length);
    JS::UTF8Chars utf8Chars(keyStr, length);
    idString.set(JS_NewStringCopyUTF8N(superGlobalContext.getJSContext(), utf8Chars));
    return JS_StringToId(superGlobalContext.getJSContext(), idString, idp);
    ...
    return JS_IndexToId(superGlobalContext.getJSContext(), keyAsInt, idp);
  } else {
    return false; // fail
  }
}
...
{
  JS::RootedIdVector props(superGlobalContext.getJSContext());
  if (!js::GetPropertyKeys(superGlobalContext.getJSContext(), *(self->jsObject), JSITER_OWNONLY, &props))
  {
...
static inline PyObject *getKey(JSObjectProxy *self, PyObject *key, JS::HandleId id, bool checkPropertyShadowsMethod) {
  ...
  for (size_t index = 0;; index++) {
    ...
    if (methodName == NULL || !PyUnicode_Check(key)) {
      JS::RootedValue value(superGlobalContext.getJSContext());
      JS_GetPropertyById(superGlobalContext.getJSContext(), *(self->jsObject), id, &value);
       ...
       */
      if (value.isObject()) {
        JS::RootedObject valueObject(superGlobalContext.getJSContext());
        JS_ValueToObject(superGlobalContext.getJSContext(), value, &valueObject);
        js::ESClass cls;
        JS::GetBuiltinClass(superGlobalContext.getJSContext(), valueObject, &cls);
        if (cls == js::ESClass::Function) {
          JS::Rooted<JS::ValueArray<1>> args(superGlobalContext.getJSContext());
          args[0].setObject(*((*(self->jsObject)).get()));
          JS::RootedValue boundFunction(superGlobalContext.getJSContext());
          if (!JS_CallFunctionName(superGlobalContext.getJSContext(), valueObject, "bind", args, &boundFunction)) {
            PyErr_Format(PyExc_SystemError, "%s JSAPI call failed", JSObjectProxyType.tp_name);
            return NULL;
          }
      ...
      return pyTypeFactory(superGlobalContext.getJSContext(), value);
    }
    else {
...
{
  JS::RootedId id(superGlobalContext.getJSContext());
  if (!keyToId(key, &id)) {
...
static inline void assignKeyValue(JSObjectProxy *self, PyObject *key, JS::HandleId id, PyObject *value) {
  if (value) { // we are setting a value
    JS::RootedValue jValue(superGlobalContext.getJSContext(), jsTypeFactory(superGlobalContext.getJSContext(), value));
    JS_SetPropertyById(superGlobalContext.getJSContext(), *(self->jsObject), id, jValue);
  } else { // we are deleting a value
    JS::ObjectOpResult ignoredResult;
    JS_DeletePropertyById(superGlobalContext.getJSContext(), *(self->jsObject), id, ignoredResult);
  }
}
...
bool JSObjectProxyMethodDefinitions::JSObjectProxy_richcompare_helper(JSObjectProxy *self, PyObject *other, std::unordered_map<PyObject *, PyObject *> &visited)
{
  ...
  if (Py_TYPE((PyObject *)self) == Py_TYPE(other)) {
    JS::RootedValue selfVal(superGlobalContext.getJSContext(), JS::ObjectValue(**(self->jsObject)));
    JS::RootedValue otherVal(superGlobalContext.getJSContext(), JS::ObjectValue(**(*(JSObjectProxy *)other).jsObject));
  ...
  for (size_t i = 0; i < length; i++)
  {
    ...
    JS::RootedValue key(superGlobalContext.getJSContext());
    key.setString(id.toString());

    PyObject *pyKey = pyTypeFactory(superGlobalContext.getJSContext(), key);
...
PyObject *JSObjectProxyMethodDefinitions::JSObjectProxy_iter(JSObjectProxy *self) {
  ...
  if (!js::GetPropertyKeys(superGlobalContext.getJSContext(), *(self->jsObject), JSITER_OWNONLY, iterator->it.props)) {
    return NULL;
  }
...
PyObject *JSObjectProxyMethodDefinitions::JSObjectProxy_repr(JSObjectProxy *self) {
  ...
  for (Py_ssize_t index = 0; index < selfLength; index++) {
    ...
    key = idToKey(superGlobalContext.getJSContext(), id);
    ...
    JS::RootedValue elementVal(superGlobalContext.getJSContext());
    JS_GetPropertyById(superGlobalContext.getJSContext(), *(self->jsObject), id, &elementVal);

    if (&elementVal.toObject() == (*(self->jsObject)).get()) {
      value = (PyObject *)self;
      Py_INCREF(value);
    } else {
      value = pyTypeFactory(superGlobalContext.getJSContext(), elementVal);
    }
...
PyObject *JSObjectProxyMethodDefinitions::JSObjectProxy_or(JSObjectProxy *self, PyObject *other) {
  ...
  if (!PyObject_TypeCheck(self, &JSObjectProxyType) && PyObject_TypeCheck(other, &JSObjectProxyType)) {
    ...
    JS::Rooted<JS::ValueArray<3>> args(superGlobalContext.getJSContext());
    args[0].setObjectOrNull(JS_NewPlainObject(superGlobalContext.getJSContext()));
    args[1].setObjectOrNull(*(self->jsObject));  // this is null is left operand is real dict
    JS::RootedValue jValueOther(superGlobalContext.getJSContext(), jsTypeFactory(superGlobalContext.getJSContext(), other));
    args[2].setObject(jValueOther.toObject());

    JS::RootedObject global(superGlobalContext.getJSContext(), JS::GetNonCCWObjectGlobal(*(self->jsObject)));

    // call Object.assign
    JS::RootedValue Object(superGlobalContext.getJSContext());
    if (!JS_GetProperty(superGlobalContext.getJSContext(), global, "Object", &Object)) {
      PyErr_Format(PyExc_SystemError, "%s JSAPI call failed", JSObjectProxyType.tp_name);
      return NULL;
    }

    JS::RootedObject rootedObject(superGlobalContext.getJSContext(), Object.toObjectOrNull());
    JS::RootedValue ret(superGlobalContext.getJSContext());

    if (!JS_CallFunctionName(superGlobalContext.getJSContext(), rootedObject, "assign", args, &ret)) {
      PyErr_Format(PyExc_SystemError, "%s JSAPI call failed", JSObjectProxyType.tp_name);
      return NULL;
    }
    return pyTypeFactory(superGlobalContext.getJSContext(), ret);
  }
}

PyObject *JSObjectProxyMethodDefinitions::JSObjectProxy_ior(JSObjectProxy *self, PyObject *other) {
  if (PyDict_Check(other)) {
    JS::Rooted<JS::ValueArray<2>> args(superGlobalContext.getJSContext());
...
PyObject *JSObjectProxyMethodDefinitions::JSObjectProxy_clear_method(JSObjectProxy *self) {
  ...
  for (size_t index = 0; index < length; index++) {
    JS_DeletePropertyById(superGlobalContext.getJSContext(), *(self->jsObject), props[index], ignoredResult);
  }
